...
async def new_member(message: types.Message):
    ...
    await message.reply(f'Guruhimizga xush kelibsiz, {members}!')
...
async def banned_member(message: types.Message):
    if message.left_chat_member.id == message.from_user.id:
        ...
        await message.answer(f'{message.left_chat_member.full_name} guruhdan '
        f'{message.from_user.get_mention(as_html=True)} tomonidan haydaldi')
